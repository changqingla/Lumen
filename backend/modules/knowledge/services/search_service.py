...
_rerank_model = None
_DeepRagPureRetriever = None
_DeepRagRetrievalConfig = None
_create_embedding_model = None
_create_rerank_model = None
...
def _load_recall_lib_dependencies() -> None:
    global _DeepRagPureRetriever, _DeepRagRetrievalConfig
    global _create_embedding_model, _create_rerank_model

    if all(
        dependency is not None
        for dependency in (
            _DeepRagPureRetriever,
            _DeepRagRetrievalConfig,
    ...
    _DeepRagPureRetriever = DeepRagPureRetriever
    _DeepRagRetrievalConfig = DeepRagRetrievalConfig
    _create_embedding_model = create_embedding_model
    _create_rerank_model = create_rerank_model


def _get_embedding_model():
    _load_recall_lib_dependencies()
...
def _get_rerank_model():
    ...
    if _rerank_model is None and settings.RERANK_FACTORY:
        _rerank_model = _create_rerank_model(
...
class SearchService:
    ...
    async def search_in_kb(
        ...
        try:
            ...
            retrieval_config = _DeepRagRetrievalConfig(
            ...
            retriever = _DeepRagPureRetriever(retrieval_config)
