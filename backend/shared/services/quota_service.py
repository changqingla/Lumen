...
from shared.services.token_usage_service import TokenUsageService
...
class QuotaService:
    ...
    def get_quota_limit(self, user: User) -> int:
        ...
        """
        # 1) 用户级数据库覆盖值优先（可用于灰度/运营临时提额）
        user_override = getattr(user, "model_quota_limit", None)
        if isinstance(user_override, int) and user_override > 0:
            return user_override

        # 2) 管理员默认使用更高上限
        if bool(getattr(user, "is_admin", False)):
            return QUOTA_LIMITS.get(UserLevel.ADMIN, 100_000_000)
