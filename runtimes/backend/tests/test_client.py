...
def client(mock_app_config):
    """Create an InsightFlowClient with mocked config loading."""
...
class TestExtractText:
    def test_string(self):
        assert InsightFlowClient._extract_text("hello") == "hello"
    ...
    def test_list_text_blocks(self):
        ...
        assert InsightFlowClient._extract_text(content) == "first\nsecond"

    def test_list_plain_strings(self):
        assert InsightFlowClient._extract_text(["a", "b"]) == "a\nb"

    def test_empty_list(self):
        assert InsightFlowClient._extract_text([]) == ""

    def test_other_type(self):
        assert InsightFlowClient._extract_text(42) == "42"
...
class TestMemoryManagement:
    ...
    def test_get_memory_config(self, client):
        config = MagicMock()
        config.enabled = True
        config.storage_path = ".lumen/memory.json"
...
class TestUploads:
    ...
    def test_delete_upload_not_found(self, client):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(InsightFlowClient, "_get_uploads_dir", return_value=Path(tmp)):
                with pytest.raises(FileNotFoundError):
...
class TestScenarioThreadIsolation:
    ...
    def test_uploads_isolated_per_thread(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(InsightFlowClient, "_get_uploads_dir", side_effect=get_dir):
...
class TestScenarioEdgeCases:
    ...
    def test_upload_convertible_file_conversion_failure(self, client):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch.object(InsightFlowClient, "_get_uploads_dir", return_value=uploads_dir),
                patch("src.gateway.routers.uploads.get_markdown_extensions", return_value={".pdf"}),
...
class TestGatewayConformance:
    ...
    def test_get_memory_config(self, client):
        ...
        mem_cfg.storage_path = ".lumen/memory.json"
