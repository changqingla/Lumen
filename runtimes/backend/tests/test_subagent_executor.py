...
class TestAsyncExecutionPath:
    ...
    def test_create_agent_reresolves_parent_model_without_passing_secret_spec(self, classes, base_config):
        ...
        with patch.object(executor_module, "create_chat_model", side_effect=_fake_create_chat_model):
            with patch.object(executor_module, "create_agent", lambda **kwargs: kwargs):
                with patch("src.config.app_config.get_app_config", return_value=MagicMock(agent_loop=MagicMock(max_identical_tool_calls=3))):
