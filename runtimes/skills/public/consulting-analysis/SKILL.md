...
When this skill produces an analysis framework, a final report, or any other user-facing file artifact, the default goal is to deliver a downloadable output, not only inline Markdown in the chat.

1. Write the generated Markdown artifact in the sandbox, for example with `sandbox_write_file`.
2. Use semantic filenames such as `documents/{topic}-analysis-framework.md` or `documents/{topic}-consulting-report.md`.
3. If the final report references chart images or other companion assets, place the report and assets in the same sandbox directory and publish that directory with `sandbox_publish_directory` so the downloaded artifact remains complete.
4. If the artifact is a single standalone Markdown file, publish it with `sandbox_publish_file`.
5. You may self-check with `sandbox_read_file`, `sandbox_read_lines`, or `sandbox_list_directory` before publishing.
6. the final response must include the published artifact delivery information and make it clear the file is available for download.
7. Do not stop after only generating inline Markdown, and do not expose internal MinIO paths in the user-facing response.
